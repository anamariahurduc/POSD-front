<template>
  <div>
    ...
    <template v-else>
      <p class="text-3xl font-semibold text-fuchsia-800 text-center mt-10">Edit invoice</p>
...
const getBillingInfo = async () => {
  await axios.get('http://api.infomed.develop.eiddew.com/api/billing-informations/' + billing_information_id)
      .then(async (response) => {
        const keyArray = hexStringToUint8Array(key.value);
        const ivArray = hexStringToUint8Array(iv.value);
